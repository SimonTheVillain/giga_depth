...
def sigma_loss(sigma, x, x_gt): #sigma_sq is also called "variance"
    ...
    eps = 0.00001 # sqrt(0.00001) = 0.0003 ... that means we actually have approx 0.3 pixel of basic offset for sigma
    term1 = 0.5 * torch.div(torch.square(x - x_gt), sigma*sigma + eps)
    term2 = 0.5 * torch.log(sigma * sigma + eps)
...
def train():
    ...
    experiment_name = "cr8_2021_32_std_4"

    writer = SummaryWriter(f"tensorboard/{experiment_name}")

    # slit loading and storing of models for Backbone and Regressor
    load_regressor = "trained_models/cr8_2021_32_std_3_regressor_chk.pt"
    load_backbone = "trained_models/cr8_2021_32_std_3_backbone_chk.pt"
    ...
    alpha_sigma = 1e-5  # how much weight do we give correct confidence measures
