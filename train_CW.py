...
def train():
    ...
    writer = SummaryWriter(path + 'tensorboard/CR_10_4hs_6')
    #writer = SummaryWriter('tensorboard/dump')

    model_path_src = path + "trained_models/CR_10_4hs_2.pt"
    load_model = True
    model_path_dst = path + "trained_models/CR_10_4hs_2.pt"
    model_path_unconditional = path + "trained_models/CR_10_4hs_3_chckpt.pt"
    ...
    alpha_regression = 50#TODO: go back to a weight of 100 for the regression part
    learning_rate = 0.02 #0.1 for the coarse part
    momentum = 0.90
