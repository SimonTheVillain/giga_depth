...
class Model_CR10_3_hsn(nn.Module):
    ...
    def __init__(self, classes, image_height, shallow=False):
        ...
        self.register_parameter(name='reg_w_1', param=nn.Parameter(torch.randn((half_height * classes, 128, 8))))
        self.register_parameter(name='reg_b_1', param=nn.Parameter(torch.randn((half_height * classes, 1, 8))))

        self.register_parameter(name='reg_w_2', param=nn.Parameter(torch.randn((half_height * classes, 8, 1))))
        self.register_parameter(name='reg_b_2', param=nn.Parameter(torch.randn((half_height * classes, 1, 1))))
    ...
    def forward(self, x, class_gt=None):
        ...
        #classes slicewise
        classes = x.transpose(1, 2)
        classes = classes.reshape((classes.shape[0], 128 * half_height, 1, classes.shape[3]))
        classes = F.leaky_relu(self.conv_end_c(classes))
        ...
        #reshape so the channels are on the last dimension
        x = x.permute((0, 2, 3, 1)).unsqueeze(3)
        x = F.leaky_relu(torch.matmul(x, w) + b)

        #second layer with custom weights
        b = self.reg_b_2.data.index_select(0, inds)
        w = self.reg_w_2.data.index_select(0, inds)
        b = b.reshape((ind_shape[0], ind_shape[2], ind_shape[3], b.shape[1], b.shape[2]))
        w = w.reshape((ind_shape[0], ind_shape[2], ind_shape[3], w.shape[1], w.shape[2]))
        x = torch.matmul(x, w) + b

        #put everything back into the right shape
