...
class Model_CR9_hsn(nn.Module):
    ...
    def __init__(self, slices, classes, image_height, pad_top=True, pad_bottom=True):
        super(Model_CR9_hsn, self).__init__()
        self.slices = slices
        ...
        for i in range(0, self.slices):
            ...
            self.conv_6.append(nn.Conv2d(128, 128, 3, padding=0, stride=1, groups=1+0*128)) # + 2 x 1 = 15
    ...
    def forward(self, x):
        ...
        for i in range(0, self.slices):
            ...
            s = F.leaky_relu(self.conv_6[i](s))
