...
class Model_CR7_n(nn.Module):

    def __init__(self, classes, image_height):
        self.classes = classes
        self.height = int(image_height / 2)
        super(Model_CR7_n, self).__init__()
        # 1 input image channel, 6 output channels, 3x3 square convolution
        # kernel
        self.conv_start = nn.Conv2d(1, 64, 3, padding=0) #1
        self.resi_block1 = ResidualBlock_shrink(64, 3, 1, depadding=0) #3
        #self.conv1 = nn.Conv2d(66, 63, 3, padding=1, padding_mode='same')
        self.resi_block2 = ResidualBlock_shrink(64, 3, 1, depadding=0) #3
        self.conv_ch_up_1 = nn.Conv2d(64, 128, 5, padding=3, stride=2) #2 here stride 2 subsampling
        self.resi_block3 = ResidualBlock_shrink(128, 5, 2, depadding=0) #3
        self.conv_end_1 = nn.Conv2d(128, 128, 7, padding=3, padding_mode='replicate') #3 or here

        # if this does not work... add one more 1x1 convolutional layer here
        self.conv_end_2 = nn.Conv2d(128 * self.height, 1024 * self.height, 1, padding=0, groups=self.height)
        self.conv_end_3 = nn.Conv2d(1024 * self.height, (classes*2+1)*self.height, 1, padding=0, groups=self.height)
