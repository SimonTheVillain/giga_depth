...
class Model_CR8_sn(nn.Module):
    @staticmethod
    def padding():
        return 1 + 3 + 3 + 2 + 6 + 3 + 0 + 0 # 18

    def __init__(self, slices, classes, image_height, pad_top=True, pad_bottom=True):
        super(Model_CR8_sn, self).__init__()
        self.slices = slices
        ...
        for i in range(0, self.slices):
            ...
            self.resi_block1.append(ResidualBlock_shrink(32, 3, 0, depadding=3)) #3
            self.resi_block2.append(ResidualBlock_shrink(32, 3, 0, depadding=3)) #3
            self.conv_ch_up_1.append(nn.Conv2d(32, 64, 5, padding=0))#-2
            self.resi_block3.append(ResidualBlock_shrink(64, 5, 0, depadding=6))#-6
            self.conv_end_1.append(nn.Conv2d(64, 128, 7, padding=0, padding_mode='replicate')) #3
    ...
    def forward(self, x):
        ...
        latent_shape = (input.shape[0], 128, int((x.shape[2] - 2 * self.r)), int(input.shape[3]))
        #print(latent_shape)
        ...
        for i in range(0, self.slices):
            ...
            intermediate[:, :, i*step:(i+1)*step, :] = s
