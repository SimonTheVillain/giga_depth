...
in_root_path = str(Path("/home/simon/datasets/structure_core/sequences_combined_ambient"))
out_root_path = str(Path("/media/simon/sandisk2/datasets/structure_core_captured_sequences_DIS"))
...
P1 = block_size ** 2 * 8
P2 = block_size ** 2 * 32
stereo = cv2.StereoSGBM_create(minDisparity=0, numDisparities=128 * 2, blockSize=block_size, P1=P1, P2=P2)
...
def generate_pcl(depth, focal, cxy):
    ...
    depth = depth.flatten()
    msk = np.logical_and(depth > 0, depth < 10)
    x = x.flatten()[msk]
    y = y.flatten()[msk]
    depth = depth[msk]
...
def process(folder, folder_ind, visualize):
    ...
    sgm_disp = np.zeros([4, 1, 448, 608], dtype=np.float32)
    R = np.zeros([4, 3, 3], dtype=np.float32)
    t = np.zeros([4, 3], dtype=np.float32)
    pcds = []
    for i in range(4):
        #print(f"{folder}_{i}")
        ir = cv2.imread(f"{in_path}/ir{i}.png", cv2.IMREAD_UNCHANGED)
        irf = (ir / (256 ** 2) ).astype(np.float32)
        irlf = irf[:, int(ir.shape[1] / 2):]
        irrf = irf[:, :int(ir.shape[1] / 2)]

        ir = (ir / 256).astype(np.ubyte)
        irl = ir[:, int(ir.shape[1] / 2):]
        irr = ir[:, :int(ir.shape[1] / 2)]

        #use semiglobal matching on these and store in "sgm_disp"
        ...
        imi = downsample(irlf)
        im[i, 0, :, :] = imi
        imri = downsample(irrf)
        imr[i, 0, :, :] = imri

        ambi = cv2.imread(f"{in_path}/ambient{i}.png", cv2.IMREAD_UNCHANGED)
        ambi = (ambi / (256 ** 2)).astype(np.float32)
        ambi = ambi[:, int(ir.shape[1] / 2):]
        ambi = downsample(ambi)
        ambient[i, 0, :, :] = ambi

        dispi = downsampleDisp(disparity) / 2.0
        disp[i, 0, :, :] = dispi
        if visualize:
            cv2.imshow("imi", imi)
            cv2.imshow("irr", irr)
        ...
        depth[depth > 10] = 0
        pcd = generate_pcl(depth, focal, tgt_cxy)
        # o3d.visualization.draw_geometries([pcd])
        pcds.append(pcd)


    # o3d.visualization.draw_geometries(pcds)
    if False:#os.path.exists(f"{in_path}/icp_results.pkl"):
    ...
    with h5py.File(f"{out_path}/frames.hdf5", "w") as f:
        ...
        f.create_dataset("sgm_disp", data=sgm_disp)
