...
class DatasetRenderedShapenet(data.Dataset):
    ...
    def __init__(self, root_dir, type="train", noise=0.1, debug=False):
        self.root_dir = root_dir
        self.use_npy = True
        self.noise = noise
        self.focal = 567.6 # approximately
        self.baseline = 0.075
        self.debug = debug
        if type == "train":
            self.from_idx = 1024
    ...
    def __getitem__(self, idx):
        ...
        if self.use_npy:
            ...
            ir = cv2.imread(f"{self.root_dir}/syn/{scene_idx:08d}/ir0_{frame_idx}.png", cv2.IMREAD_UNCHANGED)
        ...
        x_d = disp + np.expand_dims(np.arange(0, disp.shape[2]), axis=(0, 1))
